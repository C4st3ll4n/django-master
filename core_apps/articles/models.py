...
class Article(TimeStampedUUIDModel):
    ...
    def get_average_rating(self):
        if Rating.objetcs.all().count() > 0:
            rating = (
                Rating.objetcs.filter(article=self.pkid).all.aggregate(Avg('value'))
            )
...
class ArticleViews(TimeStampedUUIDModel):
    ...
    def __str__(self):
        return f"Total views on {self.article.title} is: {self.article.views}"
